...
for i in [i.split('.')[0] for i in os.listdir(".") if i.endswith(".json")]:
	...
	rdata['combat_info'] = {
		...
		"ascensions": [[{"rarity": int(i.select_one("img")['alt'].split()[1]),"image": i.select_one(".card_image img")['data-src'].split("/revision")[0],"amount": int(i.select_one(".card_text").text)} for i in i.select("td")[2:]] for i in table5.select("tr")[1:]]
	}
